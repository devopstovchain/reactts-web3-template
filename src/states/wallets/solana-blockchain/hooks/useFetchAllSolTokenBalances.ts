...
export default function useFetchAllSolTokenBalances(addressUser: string) {
    ...
    return {
        native: {
            [TokenName.SOL]: { balance: nativeSolBalance.data || BN(0), isLoading: nativeSolBalance.isLoading, isRefetching: nativeSolBalance.isFetching, refetch: nativeSolBalance.refetch },
        },
        allSlpTokenBalances,
    };
}
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            for (const tokenAccount of tokenAccounts.value) {
                ...
                const tokenName = findTokenNameSolana[token]?.name || token;
